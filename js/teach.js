...
const Teach = (() => {
  ...
  function _buildUserPrompt(userText, decision) {
    ...
    if (decision.kind === 'retry') {
      ...
      lines.push(`${_toldRestLine(decision.toldRest) || _toldLine(decision.toldNow) || `Acknowledge her reply naturally. `}Then let her know all five key points in this lesson have now been covered, and close warmly in one short line. Do not ask a question. Do not offer practice questions. She can ask you anything further, or leave, whenever she likes.`);
    } else if (_state.wrapOffered) {
      lines.push(`All five key points in this lesson have already been covered, and you have ALREADY asked her whether she wants to go over anything else. Do not ask that again in any form — no "anything else?", no "does that feel clear?", no "shall we stop there?". Respond to what she actually said and then stop. If she shows she is finished (no, nothing, bye, thanks, I'm good), give ONE warm, complete goodbye and nothing after it — no question of any kind. A light sign-off such as "same time next week?" is fine as that closing line itself, but only as the goodbye.`);
    } else {
      lines.push(`All five key points in this lesson have already been covered. Respond naturally and helpfully to whatever she said. Do not offer practice questions and do not present lettered or numbered options. Ask at most one closing question, and only if it is genuinely useful.`);
    }
